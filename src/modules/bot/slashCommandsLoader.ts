...
export const loadSlashCommands = async (): Promise<
  Collection<string, SlashCommand>
> => {
  const commands = new Collection<string, SlashCommand>();

  const slashCommandsPath = path.join(process.cwd(), process.argv.includes('dev') ? 'src' : 'build', 'commands');
  ...
  for (const slashCommandFile of slashCommandsFiles) {
    const slashCommandImported = (await import(
      `${slashCommandsPath}/${slashCommandFile}`
    )) as SlashCommand;
